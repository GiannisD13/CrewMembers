...
export default function JobCard({ job, compact = false }: JobCardProps) {
  ...
  return (
    <div className="group bg-navy-light border border-white/6 rounded-2xl overflow-hidden transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl hover:shadow-black/50 hover:border-gold/25 cursor-pointer flex flex-col">
      ...
      {!compact && (
        <div className={`relative h-40 bg-gradient-to-br ${bg} flex items-center justify-center`}>
          ...
          {job.urgent && (
            <span className="absolute top-3 right-3 z-10 bg-gold text-navy text-[10px] font-bold tracking-widest px-2.5 py-1 rounded uppercase">
              Urgent
            </span>
          )}
        </div>
      ...
      <div className="p-5 flex flex-col flex-1">
        {/* Title */}
        <div className="mb-1">
          <h3 className="font-display text-[1.05rem] font-semibold text-cream leading-tight group-hover:text-gold-light transition-colors">
            {job.vesselName}
          </h3>
        ...
        {/* Route */}
        <div className="flex items-center gap-1.5 my-3 text-xs text-cream/60">
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {job.rolesNeeded.map(role => (
            <span
              key={role}
              className="text-[11px] font-medium px-2.5 py-0.5 rounded-full bg-teal/15 text-teal-light border border-teal/25"
            >
              {role}
            </span>
          ))}
        ...
        <div className="mt-auto flex items-center justify-between pt-3.5 border-t border-white/6">
          <div>
            <p className="text-[11px] text-cream/40 mb-0.5">Departure window</p>
